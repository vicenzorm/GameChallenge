...
class CutscenePlayer: NSObject {
    ...
    func play(wave: Int, over view: SKView, onFinished: @escaping () -> Void) {
        self.onFinished = onFinished
        guard let scene = view.scene else { onFinished(); return }
        setupOverlay(nextFloor: wave + 1, in: scene)
    }

    // MARK: - Setup

    private func setupOverlay(nextFloor: Int, in scene: SKScene) {
        let root = SKNode()
        root.zPosition = 500   // acima de tudo, inclusive do HUD
        self.overlayNode = root

        // Fundo preto cobrindo toda a tela
        let bg = SKShapeNode(rectOf: scene.size)
        bg.fillColor   = .black
        ...
        floorLabel.position     = CGPoint(x: 0, y: 20)
        floorLabel.zPosition    = 1
        ...
        // Adiciona ao camera node se existir, senão direto na cena
        ...
        // Animação: fade in → espera → fade out → finish
        root.alpha = 0
        root.run(.sequence([
...
//
//    // ── Arquivo de vídeo ──────────────────────────────────────────
//    private static let videoExtension = "mov"   // ← extensão do vídeo
//    private static func videoName(forWave wave: Int) -> String {
//        return "cutscene1"           // ← padrão do nome (sem extensão)
//    }
//
//    // ── Privados ──────────────────────────────────────────────────
